...
// gal_error_code gal_impl_vk::result_to_gal_error_code(VkResult res) {
...
gal_error_code gal_impl_vk::init_gal(gal_context *context) {
...
gal_error_code gal_impl_vk::destroy_gal(gal_context context) {
    if (context != gal_null) {
        vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
        bd::memory::afree(vk_ctx);
        context = gal_null;
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::create_instance(gal_desc *gal_desc, gal_context *context) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(*context);
    vk_ctx->m_gal_desc = *gal_desc;

    stl::vector<const char *> required_instance_layers;
    stl::vector<const char *> required_instance_extensions;
    ...
    stl::vector<VkLayerProperties> available_layers(available_layer_count);
    stl::vector<VkExtensionProperties> available_extensions(available_extension_count);

    vkEnumerateInstanceLayerProperties(&available_layer_count, available_layers.data());
    vkEnumerateInstanceExtensionProperties(nullptr, &available_extension_count, available_extensions.data());
    uint32 required_layer_count = static_cast<uint32>(required_instance_layers.size());
    uint32 required_extension_count = static_cast<uint32>(required_instance_extensions.size());

    for (uint32 i = 0; i < available_layer_count; i++) {
        for (auto &required_instance_layer : required_instance_layers) {
            if (std::strcmp(required_instance_layer, available_layers[i].layerName) == 0) {
    ...
    for (uint32 i = 0; i < available_extension_count; i++) {
        for (auto &required_instance_extension : required_instance_extensions) {
            if (std::strcmp(required_instance_extension, available_extensions[i].extensionName) == 0) {
    ...
    instance_create_info.enabledExtensionCount = static_cast<uint32>(required_instance_extensions.size());
    instance_create_info.ppEnabledExtensionNames = required_instance_extensions.data();
    instance_create_info.enabledLayerCount = static_cast<uint32>(required_instance_layers.size());
...
gal_error_code gal_impl_vk::destroy_instance(gal_context *context) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(*context);

    vkDestroyInstance(vk_ctx->instance, nullptr);
    return gal_error_code::SUC;
}
gal_error_code gal_impl_vk::create_device(gal_desc *gal_desc, gal_context *context) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(*context);

    //

    stl::vector<const char *> required_device_extensions;
    stl::vector<const char *> required_device_layers;
    ...
    stl::vector<VkPhysicalDevice> physical_devices(physical_device_count);
    if (physical_device_count == 0) {
        Log::log(gal_vk_logger, loglevel::error, "no available device");
        return gal_error_code::ERR;
    }
    vkEnumeratePhysicalDevices(vk_ctx->instance, &physical_device_count, physical_devices.data());

    uint32 required_queue_count = 1u;
    ...
    // pick gpu
    auto pick_gpu = [&](stl::array<uint32, 3> &queue_family_indices) -> gal_error_code {
        for (const auto &physical_device : physical_devices) {
            ...
            stl::vector<VkQueueFamilyProperties> queue_family_properties(queue_family_count);

            queue_family_properties.resize(queue_family_count);
            vkGetPhysicalDeviceQueueFamilyProperties(physical_device, &queue_family_count,
                                                     queue_family_properties.data()); // Get queue family properties

            auto getQueueFamilyIndex = [&](gal_queue_type queue_type) -> std::optional<uint32> {
                ...
                if (queue_type == gal_queue_type::compute) {
                    for (uint32 i = 0; i < static_cast<uint32>(queue_family_properties.size()); i++) {
                        if (((queue_family_properties[i].queueFlags & VK_QUEUE_COMPUTE_BIT) != 0u) &&
                ...
                if (queue_type == gal_queue_type::transfer) {
                    for (uint32 i = 0; i < static_cast<uint32>(queue_family_properties.size()); i++) {
                        if (((queue_family_properties[i].queueFlags & VK_QUEUE_TRANSFER_BIT) != 0u) &&
                            ((queue_family_properties[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) == 0) &&
                ...
                Log::log(gal_vk_logger, loglevel::warn, "request queue not found, fallback to graphcis queue");
                return {};
            };
            ...
            stl::vector<VkLayerProperties> available_device_layers(available_layer_count);
            ...
            stl::vector<VkExtensionProperties> available_device_extensions(available_extension_count);
            vkEnumerateDeviceExtensionProperties(physical_device, layer_name, &available_extension_count,
                                                 available_device_extensions.data());

            uint32 required_layer_count = static_cast<uint32>(required_device_layers.size());
            uint32 required_extension_count = static_cast<uint32>(required_device_extensions.size());
            for (uint32 i = 0; i < available_layer_count; i++) {
                for (auto &required_device_layer : required_device_layers) {
                    if (std::strcmp(required_device_layer, available_device_layers[i].layerName) == 0) {
                        required_layer_count--;
                    }
                }
            }
            ...
            for (uint32 i = 0; i < available_extension_count; i++) {
                for (auto &required_device_extension : required_device_extensions) {
                    if (std::strcmp(required_device_extension, available_device_extensions[i].extensionName) == 0) {
                        required_extension_count--;
                    }
                }
            }
    ...
    stl::array<uint32, 3> queue_family_indices;
    ...
    shader_draw_parameters_features.shaderDrawParameters = 1u;
    ...
    dyanmic_rendering_features.dynamicRendering = 1u;
    ...
    stl::set<uint32> queue_family_indices_set(queue_family_indices.begin(), queue_family_indices.end());
    stl::vector<VkDeviceQueueCreateInfo> device_queue_create_info(queue_family_indices_set.size());
    uint32 idx = 0;
    stl::array<uint32, 3> required_queue_family_indices;
    for (const auto &index : queue_family_indices_set) {
        required_queue_family_indices[idx++] = index;
    }

    for (uint32 i = 0; i < static_cast<uint32>(queue_family_indices_set.size()); i++) {
    ...
    device_create_info.queueCreateInfoCount = static_cast<uint32>(device_queue_create_info.size());
    device_create_info.enabledExtensionCount = static_cast<uint32>(required_device_extensions.size());
    device_create_info.ppEnabledExtensionNames = required_device_extensions.data();
    device_create_info.enabledLayerCount = static_cast<uint32>(required_device_layers.size());
...
gal_error_code gal_impl_vk::destroy_device(gal_context *context) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(*context);
...
gal_error_code gal_impl_vk::create_memory_allocator(gal_context *context) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(*context);
...
gal_error_code gal_impl_vk::destroy_memory_allocator(gal_context *context) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(*context);
...
gal_error_code gal_impl_vk::create_buffer(gal_context context, gal_buffer_desc *desc, gal_buffer *buffer) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_buffer *vk_buf = bd::memory::alloc<gal::vk_buffer>();
    if (vk_buf == nullptr) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::destroy_buffer(gal_context context, gal_buffer buffer) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    ...
    if (buffer != gal_null) {
        ...
        bd::memory::afree(vk_buf);
        buffer = gal_null;
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::create_texture(gal_context context, gal_texture_desc *desc, gal_texture *texture) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_texture *vk_tex = bd::memory::alloc<gal::vk_texture>();

    if (vk_tex == nullptr) {
        return gal_error_code::ERR;
    }

    uint32 array_size = desc->array_size > 1u ? desc->array_size : 1u;
    uint32 depth = desc->depth > 1u ? desc->depth : 1u;
    bool b_is_cubemap =
    ...
    if ((format_props.optimalTilingFeatures & format_features) == 0u) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::destroy_texture(gal_context context, gal_texture texture) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    if (texture != gal_null) {
        ...
        bd::memory::afree(vk_tex);
        texture = gal_null;
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::create_sampler(gal_context context, gal_sampler_desc *sampler_desc, gal_sampler *sampler) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_sampler *vk_spl = bd::memory::alloc<gal::vk_sampler>();
    ...
    sampler_create_info.compareEnable =
        static_cast<VkBool32>(sampler_desc->compare_mode != gal_compare_mode::UNDEFINED);
...
gal_error_code gal_impl_vk::destroy_sampler(gal_context context, gal_sampler sampler) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    if (sampler != gal_null) {
        vk_sampler *vk_spl = reinterpret_cast<vk_sampler *>(sampler);
        vkDestroySampler(vk_ctx->device, vk_spl->m_sampler, nullptr);
        bd::memory::afree(vk_spl);
        sampler = gal_null;
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::create_render_target(gal_context context, gal_render_target_desc *desc,
                                                 gal_render_target *render_target) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_render_target *vk_rt = bd::memory::alloc<vk_render_target>();
    ...
    if ((desc->flags & gal_texture_flag::TEXTURE_CREATION_FLAG_ON_TILE) ==
        ...
        if ((textureDesc.descriptor_types & gal_descriptor_type::TEXTURE) != gal_descriptor_type::UNDEFINED ||
            ...
            Log::log(gal_vk_logger, loglevel::warn,
                     "On tile textures do not support DESCRIPTOR_TYPE_TEXTURE or DESCRIPTOR_TYPE_RW_TEXTURE");
        }
    ...
    if (isDepth) {
        ...
        if (VK_FORMAT_UNDEFINED != vk_depth_stencil_format) {
            ...
            if (VK_SUCCESS != vk_res) {
                ...
                Log::log(gal_vk_logger, loglevel::warn,
                         "Depth stencil format (%u) not supported. Falling back to D32 format", desc->height);
            }
        }
    }
    // gal_create_texture
    ...
    vk_rt->pVkSliceDescriptors = static_cast<VkImageView *>(memory::amalloc(desc->mip_level * sizeof(VkImageView)));

    for (uint32 i = 0; i < desc->mip_level; ++i) {
        ...
        //    for (uint32 j = 0; j < depthOrArraySize; ++j) {
        //        rtvDesc.subresourceRange.layerCount = 1;
...
gal_error_code gal_impl_vk::destroy_render_target(gal_context context, gal_render_target render_target) {
...
// surface
gal_error_code gal_impl_vk::create_swap_chain(gal_context context, gal_swap_chain_desc *desc,
                                              gal_swap_chain *swap_chain) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_swap_chain *vk_sc = bd::memory::alloc<vk_swap_chain>();
    ...
    add_info.flags = 0;
    add_info.hinstance = ::GetModuleHandle();
...
#error PLATFORM NOT SUPPORTED
    ...
    if ((caps.maxImageCount > 0) && (desc->image_count > caps.maxImageCount)) {
        Log::log(gal_vk_logger, loglevel::warn,
                 "Changed requested SwapChain images {} to maximum allowed SwapChain images {}", desc->image_count,
                 caps.maxImageCount);
        desc->image_count = caps.maxImageCount;
    }
    if (desc->image_count < caps.minImageCount) {
        Log::log(gal_vk_logger, loglevel::warn,
                 "Changed requested SwapChain images {} to minimum required SwapChain images {}", desc->image_count,
                 caps.minImageCount);
    ...
    stl::vector<VkPresentModeKHR> modes;
    ...
    const uint32 preferredModeCount = 4;
    stl::array<VkPresentModeKHR, preferredModeCount> preferredModeList = {
        VK_PRESENT_MODE_IMMEDIATE_KHR, VK_PRESENT_MODE_MAILBOX_KHR, VK_PRESENT_MODE_FIFO_RELAXED_KHR,
        VK_PRESENT_MODE_FIFO_KHR};
    uint32 preferredModeStartIndex = desc->b_vsync ? 2 : 0;

    for (uint32 j = preferredModeStartIndex; j < preferredModeCount; ++j) {
    ...
    stl::array<VkCompositeAlphaFlagBitsKHR, 4> compositeAlphaFlags = {
    ...
    for (uint32 i = 0; i < imageCount; ++i) {
        descColor.native_handle = reinterpret_cast<void *>(vk_sc->swap_chain_images[i]);
        create_render_target(context, &descColor, &vk_sc->get_render_targets()[i]);
    }
...
gal_error_code gal_impl_vk::destroy_swap_chain(gal_context context, gal_swap_chain swap_chain) {
    if (swap_chain != gal_null) {
        ...
        bd::memory::afree(vk_sc);
        swap_chain = gal_null;
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::create_shader_program(gal_context context, gal_shader_program_desc *desc,
                                                  gal_shader_program *shader_program) {

    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_shader_program *vk_sp = bd::memory::alloc<gal::vk_shader_program>();
    if (vk_sp == nullptr) {
        return gal_error_code::ERR;
    }
    ...
    auto create_shader_module = [&](compiled_shader *shader, shader_stage_index idx) {
        ...
        create_info.pCode = static_cast<const uint32 *>(shader->byte_code()->data());
        result = vkCreateShaderModule(vk_ctx->device, &create_info, nullptr,
                                      &vk_sp->m_shader_modules[static_cast<uint32>(idx)]);
        if (result != VK_SUCCESS) {
            return gal_error_code::ERR;
        }
        vk_sp->m_entrys[static_cast<uint32>(idx)] = shader->entry();
...
gal_error_code gal_impl_vk::destroy_shader_program(gal_context context, gal_shader_program shader_program) {
    if (shader_program != gal_null) {
        ...
        bd::memory::afree(vk_sp);
        shader_program = gal_null;
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::create_pipeline_cache(gal_context context, gal_pipeline_cache_desc *desc,
                                                  gal_pipeline_cache *pipeline_cache) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_pipeline_cache *vk_pc = bd::memory::alloc<gal::vk_pipeline_cache>();
    if (vk_pc == nullptr) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::destroy_pipeline_cache(gal_context context, gal_pipeline_cache _pipeline_cache) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    if (_pipeline_cache != gal_null) {
        ...
        bd::memory::afree(vk_pc);
        _pipeline_cache = gal_null;
    }
    return gal_error_code::SUC;
}
gal_error_code gal_impl_vk::get_pipeline_cache_data(gal_context context, gal_pipeline_cache _pipeline_cache,
                                                    uint64 *_size, void *_data) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_pipeline_cache *vk_pc = reinterpret_cast<vk_pipeline_cache *>(_pipeline_cache);
...
gal_error_code gal_impl_vk::create_compute_pipeline(gal_context context, gal_pipeline_desc *desc,
                                                    gal_pipeline *pipeline) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_pipeline *vk_pipe = bd::memory::alloc<gal::vk_pipeline>();
    ...
    shader_stage_create_info.module = vk_sp->m_shader_modules[static_cast<uint32>(shader_stage_index::COMP)];
    shader_stage_create_info.pName = vk_sp->m_entrys[static_cast<uint32>(shader_stage_index::COMP)];
    ...
    VkPipelineCache pipeline_cache = desc->pipeline_cache != nullptr
...
gal_error_code gal_impl_vk::create_graphics_pipeline(gal_context context, gal_pipeline_desc *desc,
                                                     gal_pipeline *pipeline) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_pipeline *vk_pipe = bd::memory::alloc<gal::vk_pipeline>();
    if (vk_pipe == nullptr) {
    ...
    uint32 stage_count = 0;
    stl::array<VkPipelineShaderStageCreateInfo, 5> stages;
    ...
    uint32 input_binding_count = 0;
    stl::array<VkVertexInputBindingDescription, MAX_VERTEX_BINDINGS> input_bindings{};
    uint32 input_attribute_count = 0;
    stl::array<VkVertexInputAttributeDescription, MAX_VERTEX_ATTRIBS> input_attributes{};
    ...
    if (pVertexLayout != nullptr) {
        ...
        uint32 binding_value = UINT32_MAX;
    ...
    vi.pVertexAttributeDescriptions = input_attributes.data();
    ...
    rs = gfx_desc.pRasterizerState != nullptr ? util_to_rasterizer_desc(gfx_desc.pRasterizerState)
    ...
    stl::array<VkPipelineColorBlendAttachmentState, MAX_RENDER_TARGET_ATTACHMENTS> cbAtt{};
    VkPipelineColorBlendStateCreateInfo cb = util_to_blend_desc(gfx_desc.pBlendState, cbAtt.data());
    cb.attachmentCount = gfx_desc.mRenderTargetCount;

    stl::array<VkDynamicState, 5> dyn_states = {
...
//gal_error_code gal_impl_vk::create_raytracing_pipeline(gal_context context, gal_raytracing_pipeline_desc *desc,
//                                           gal_pipeline *pipeline) {
//    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
//    *pipeline = reinterpret_cast<gal_pipeline>(new gal::vk_pipeline);
...
gal_error_code gal_impl_vk::destroy_pipeline(gal_context context, gal_pipeline pipeline) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
...
gal_error_code gal_impl_vk::get_descriptor_set(gal_context context, gal_descriptor_set_desc *desc, uint32 set_count,
                                               gal_descriptor_set *sets) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_root_signature *vk_rs = reinterpret_cast<vk_root_signature *>(desc->root_signature);
    //vk_descriptor_set *vk_ds = bd::memory::alloc<gal::vk_descriptor_set>();

    uint32 set_index = desc->set.index;
    if (set_index > MAX_DESCRIPTOR_SET_COUNT) {
        return gal_error_code::ERR;
    }
    if (vk_rs->set_layouts[set_index] == VK_NULL_HANDLE) {
        return gal_error_code::ERR;
    }
    vk_descriptor_set *vk_ds =
        static_cast<gal::vk_descriptor_set *>(bd::memory::amalloc(set_count * sizeof(gal::vk_descriptor_set)));
    if (vk_ds == nullptr) {
        return gal_error_code::ERR;
    }

    stl::vector<VkDescriptorSet> vk_sets;
    for (uint32 i = 0; i < set_count; i++) {
        vk_sets.push_back(vk_ds[i].set);
    }
    stl::vector<VkDescriptorSetLayout> set_layouts;
    ...
    if (set_index == static_cast<uint32>(gal_descriptor_set_update_freq::BINDLESS)) {
...
gal_error_code gal_impl_vk::free_descriptor_set(gal_context context, gal_descriptor_set set, bool free_all_pool) {
...
gal_error_code gal_impl_vk::update_descriptor_set(gal_context context, gal_descriptor_set_update_desc *update_desc,
                                                  gal_descriptor_set set) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_descriptor_set *vk_ds = reinterpret_cast<vk_descriptor_set *>(set);

    stl::vector<VkDescriptorUpdateData> descriptor_updates;
    descriptor_updates.resize(update_desc->count);
    for (uint32 i = 0; i < update_desc->count; i++) {
        ...
        uint32 set_bind_bind = iter->second;
        uint32 bind_order = (set_bind_bind & 0x7E0000) >> 17;
        //uint32 bind_index = (set_bind_bind & (64 << 23));
        //uint32 set_index = (set_bind_bind >> 29) & 0xF;
        uint32 set_index = (set_bind_bind & 0xE0000000) >> 29;
        ...
        descriptor_updates[bind_order] = update_data;
    }
...
// FIXME(hyl5): this function is too messy
gal_error_code gal_impl_vk::create_root_signature(gal_context context, gal_root_signature_desc *desc,
                                                  gal_root_signature *root_signature) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_root_signature *vk_rs = bd::memory::alloc<gal::vk_root_signature>();
    if (vk_rs == nullptr) {
        return gal_error_code::ERR;
    }
    ...
    uint32 set_count = static_cast<uint32>(refl->sets.size());
    // TODO(hyl5): seperate descriptor set layouts for caching

    if (set_count > MAX_DESCRIPTOR_SET_COUNT) {
        Log::log(gal_vk_logger, loglevel::error, "maximum of descriptor set count is: {}, current is: {}",
                 MAX_DESCRIPTOR_SET_COUNT, set_count);
        return gal_error_code::ERR;
    }

    stl::array<VkDescriptorSetLayoutCreateInfo, MAX_DESCRIPTOR_SET_COUNT> set_layouts_ci;
    //stl::array<uint32, MAX_DESCRIPTOR_SET_COUNT> set_binding_offsets;
    stl::vector<VkDescriptorSetLayoutBinding> bindings;
    //stl::vector<uint32> binding_prefix_sum(refl->sets.size(), );
    stl::vector<uint32> binding_offsets(refl->sets.size());
    uint32 total_binding_count = 0;
    stl::map<uint32, uint32> set_index_map;
    for (uint32 i = 0; i < refl->sets.size(); i++) {
        uint32 set_index = (refl->sets[i] >> 16) & 0x0000ffff; // first 16bit
        uint32 set_binding_count = refl->sets[i] & 0x0000ffff; // last 16bit
    ...
    stl::vector<VkPushConstantRange> push_constants;
    ...
    for (auto &resource : refl->m_resources) {
        if (resource.resource_type == ShaderResourceType::RESOURCE) {
            ...
            bindings[binding_offsets[set_index_map[resource.set]]] = binding;
    ...
    stl::vector<stl::hash_map<VkDescriptorType, uint32>> pool_sizes;
    ...
    for (auto &set : refl->sets) {
        uint32 set_index = (set >> 16) & 0x0000ffff; // first 16bit
        // TODO(hyl5): don't forget to free
        VkDescriptorPoolSize *vk_pool_sizes = static_cast<VkDescriptorPoolSize *>(
            bd::memory::amalloc(pool_sizes[set_index_map[set_index]].size() * sizeof(VkDescriptorPoolSize)));
        ...
        vk_rs->descriptor_pool_size[set_index].pool_size_count =
            static_cast<uint32>(pool_sizes[set_index_map[set_index]].size());
        vk_rs->descriptor_pool_size[set_index].pool_sizes = vk_pool_sizes;
    }
    uint32 required_set_count = 0;
    if (!refl->sets.empty()) {
        uint32 max_set_index = (*std::max_element(refl->sets.begin(), refl->sets.end(),
                                                  [](uint32 a, uint32 b) {
                                                      return ((a >> 16) & 0x0000ffff) < ((b >> 16) & 0x0000ffff);
                                                  }) >>
                                16) &
                               0x0000ffff;
        auto utils_fill_empty_descriptor_sets = [&]() {
            ...
            for (uint32 i = 0; i <= max_set_index; i++) {
    ...
    plci.pushConstantRangeCount = static_cast<uint32>(push_constants.size());
    ...
    //stl::vector<VkDescriptorUpdateTemplateEntry> descriptor_update_template_entries();
    //descriptor_update_template_entries.resize(total_binding_count);
    stl::vector<stl::vector<VkDescriptorUpdateTemplateEntry>> descriptor_update_template_entries;
    descriptor_update_template_entries.resize(refl->sets.size());
    //uint32 offsets = 0;

    //auto get_offset = [](VkDescriptorType type) -> uint32 {
    ...
    for (auto &resource : refl->m_resources) {
        if (resource.resource_type == ShaderResourceType::RESOURCE) {
            uint32 set_order = set_index_map[resource.set];
            ...
            descriptor_update_template_entries[set_order].push_back(entry);
        }
    }
    for (unsigned int set : refl->sets) {
        uint32 set_index = (set >> 16) & 0x0000ffff; // first 16bit
        ...
        dsut_ci.descriptorUpdateEntryCount =
            static_cast<uint32>(descriptor_update_template_entries[set_index_map[set_index]].size());
...
gal_error_code gal_impl_vk::destroy_root_signature(gal_context context, gal_root_signature root_signature) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
...
// sync
gal_error_code gal_impl_vk::create_fence(gal_context context, gal_fence *fence) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_fence *vk_f = bd::memory::alloc<vk_fence>();
    if (vk_f == nullptr) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::wait_fences(gal_context context, gal_fence *fences, uint32 count) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);

    stl::vector<VkFence> submit_fences;
    for (uint32 i = 0; i < count; ++i) {
        if (reinterpret_cast<vk_fence *>(fences[i])->b_submitted) {
    ...
    if (!submit_fences.empty()) {
        result = vkWaitForFences(vk_ctx->device, static_cast<uint32>(submit_fences.size()), submit_fences.data(),
                                 VK_TRUE, UINT64_MAX);
        if (result != VK_SUCCESS) {
            return gal_error_code::ERR;
        }
        result = vkResetFences(vk_ctx->device, static_cast<uint32>(submit_fences.size()), submit_fences.data());
        if (result != VK_SUCCESS) {
            return gal_error_code::ERR;
        }
    }
...
gal_error_code gal_impl_vk::getFenceStatus(gal_context context, gal_fence fence, gal_fence_status *fence_status) {
...
gal_error_code gal_impl_vk::destroy_fence(gal_context context, gal_fence fence) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    if (fence != gal_null) {
        vk_fence *vk_f = reinterpret_cast<vk_fence *>(fence);
        vkDestroyFence(vk_ctx->device, vk_f->fence, nullptr);
        bd::memory::afree(vk_f);
        fence = gal_null;
    }
    return gal_error_code::SUC;
}
//gal_error_code gal_impl_vk::wait_gpu() {
//    //vkDeviceWaitIdle();
//    return gal_error_code::SUC;
//}
gal_error_code gal_impl_vk::create_semaphore(gal_context context, gal_semaphore *semaphore) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_semaphore *vk_s = bd::memory::alloc<vk_semaphore>();
    if (vk_s == nullptr) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::destroy_semaphore(gal_context context, gal_semaphore semaphore) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    if (semaphore != gal_null) {
        ...
        bd::memory::afree(vk_s);
        semaphore = gal_null;
    }
    return gal_error_code::SUC;
}
// cmds
gal_error_code gal_impl_vk::create_command_pool(gal_context context, gal_command_pool_desc *desc,
                                                gal_command_pool *command_pool) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_command_pool *vk_cmd_pool = bd::memory::alloc<gal::vk_command_pool>();
    if (vk_cmd_pool == nullptr) {
        return gal_error_code::ERR;
    }
    ...
    if (!vk_ctx->queues[utils_to_vk_queue_index(desc->queue_type)].used) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::reset_command_pool(gal_context context, gal_command_pool command_pool) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
...
gal_error_code gal_impl_vk::destroy_command_pool(gal_context context, gal_command_pool command_pool) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    if (command_pool != gal_null) {
        vk_command_pool *vk_cmd_pool = reinterpret_cast<vk_command_pool *>(command_pool);
        vkDestroyCommandPool(vk_ctx->device, vk_cmd_pool->m_cmd_pool, nullptr);
        bd::memory::afree(vk_cmd_pool);
        command_pool = gal_null;
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::get_command_list(gal_context context, gal_command_list_desc *desc,
                                             gal_command_list *command) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_command_list *vk_cmd = bd::memory::alloc<gal::vk_command_list>();
...
gal_error_code gal_impl_vk::free_command_list(gal_context context, gal_command_list command) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    ...
    if (command != gal_null) {
        ...
        vkFreeCommandBuffers(vk_ctx->device, reinterpret_cast<vk_command_pool *>(vk_cmd->m_cmd_pool)->m_cmd_pool, 1,
                             &vk_cmd->m_command);
        bd::memory::afree(vk_cmd);
        command = gal_null;
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::mapbuffer(gal_context context, gal_buffer buffer, read_range *range) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_buffer *vk_buf = reinterpret_cast<vk_buffer *>(buffer);
    ...
    if (range != nullptr) {
        vk_buf->cpu_mapped_address = static_cast<uint8 *>(vk_buf->cpu_mapped_address) + range->offset;
    }
...
gal_error_code gal_impl_vk::unmapbuffer(gal_context context, gal_buffer buffer) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_buffer *vk_buf = reinterpret_cast<vk_buffer *>(buffer);
    if (buffer->m_gal_buffer_desc.memory_flags != gal_memory_flag::CPU_UPLOAD &&
        buffer->m_gal_buffer_desc.memory_flags != gal_memory_flag::GPU_DOWNLOAD) {
...
// TODO(hylu) : gal_error_code gal_impl_vk::create_command_list;

gal_error_code gal_impl_vk::cmd_begin(gal_command_list command) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code gal_impl_vk::cmd_end(gal_command_list command) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code gal_impl_vk::cmd_set_render_target() { return gal_error_code::SUC; }
gal_error_code gal_impl_vk::cmd_set_viewport(gal_command_list command, float32 x, float32 y, float32 width,
                                             float32 height, float32 min_depth, float32 max_depth) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code gal_impl_vk::cmd_set_scissor(gal_command_list command, int32 x, int32 y, uint32 width, uint32 height) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code gal_impl_vk::cmd_set_stencil_reference_value(gal_command_list command, uint32 val) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code gal_impl_vk::cmd_resource_barrier(gal_command_list command, uint32 buffer_barrier_count,
    ...
    stl::vector<VkBufferMemoryBarrier> bbs;

    for (uint32 i = 0; i < buffer_barrier_count; i++) {
    ...
    stl::vector<VkImageMemoryBarrier> tbs;

    for (uint32 i = 0; i < texture_barrier_count; i++) {
    ...
    if (!bbs.empty() || !tbs.empty()) {
        vkCmdPipelineBarrier(vk_cmd->m_command, srcStageMask, dstStageMask, 0, 0, nullptr,
                             static_cast<uint32>(bbs.size()), bbs.data(), static_cast<uint32>(tbs.size()), tbs.data());
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::cmd_bind_index_buffer(gal_command_list command, gal_buffer index_buffer,
...
gal_error_code gal_impl_vk::cmd_bind_vertex_buffer(gal_command_list command, uint32 vertex_buffer_count,
                                                   gal_buffer *vertex_buffers, [[maybe_unused]] uint32 *strides,
                                                   uint64 *offsets) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);

    if (vertex_buffer_count > MAX_VERTEX_BUFFER_BINDING) {
        Log::log(gal_vk_logger, loglevel::error, "cannot bind more than {} vertex buffers, current is: {}",
                 MAX_VERTEX_BUFFER_BINDING, vertex_buffer_count);
        return gal_error_code::ERR;
    }

    stl::array<VkBuffer, MAX_VERTEX_BUFFER_BINDING> buffers;
    stl::array<VkDeviceSize, MAX_VERTEX_BUFFER_BINDING> vb_offsets;
    for (uint32 i = 0; i < vertex_buffer_count; i++) {
        if (vertex_buffers[i] == nullptr) {
            Log::log(gal_vk_logger, loglevel::error, "invalid vertex buffers");
            return gal_error_code::ERR;
        }
        buffers[i] = reinterpret_cast<vk_buffer *>(vertex_buffers[i])->m_buffer;
        vb_offsets[i] = (offsets != nullptr ? offsets[i] : 0);
    }

    vkCmdBindVertexBuffers(vk_cmd->m_command, 0, vertex_buffer_count, buffers.data(), vb_offsets.data());
    return gal_error_code::SUC;
}
gal_error_code gal_impl_vk::cmd_bind_descriptor_set() {
    //vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code gal_impl_vk::cmd_bind_push_constant(gal_command_list command, gal_root_signature root_signature,
                                                   const char *name, void *data, uint64 size) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
    vk_root_signature *vk_rs = reinterpret_cast<vk_root_signature *>(root_signature);
    // cannot find resource or resource is not push constant
    auto it = vk_rs->resource_map.find(stl::str(name));
    if (it == vk_rs->resource_map.end() || ((it->second >> 16) & 0x01) == 0) {
        return gal_error_code::ERR;
    }
    vkCmdPushConstants(vk_cmd->m_command, vk_rs->pipeline_layout, VK_SHADER_STAGE_ALL, 0, static_cast<uint32>(size),
                       data);
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::cmd_bind_pipeline(gal_command_list command, gal_pipeline pipeline) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code gal_impl_vk::cmd_begin_renderpass(gal_command_list command, gal_renderpass_begin_desc *desc) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
    if (desc != nullptr) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::cmd_end_renderpass(gal_command_list command) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);

    vkCmdEndRendering(vk_cmd->m_command);
    return gal_error_code::SUC;
}
gal_error_code gal_impl_vk::cmd_dispatch(gal_command_list command, uint32 group_count_x, uint32 group_count_y,
...
gal_error_code gal_impl_vk::cmd_draw_instanced(gal_command_list command, uint32 vertex_count, uint32 first_vertex,
...
gal_error_code gal_impl_vk::cmd_draw_indexed_instanced(gal_command_list command, uint32 index_count, uint32 first_index,
                                                       uint32 instance_count, uint32 first_instance,
                                                       uint32 first_vertex) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);

    vkCmdDrawIndexed(vk_cmd->m_command, index_count, instance_count, first_index, static_cast<int32>(first_vertex),
                     first_instance);
    return gal_error_code::SUC;
}

//gal_error_code gal_impl_vk::cmd_dispatch_indirect() {
//    //vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
//    //vkCmdDispatchIndirect();
//    return gal_error_code::SUC;
//}
//gal_error_code gal_impl_vk::cmd_draw_indirect_instanced() {
//    //vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
//    //vkCmdDrawIndirect();
//    return gal_error_code::SUC;
//}
//gal_error_code gal_impl_vk::cmd_draw_indirect_indexed_instanced() {
//    //vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
//    //vkCmdDrawIndexedIndirect();
//    return gal_error_code::SUC;
//}
//
//gal_error_code gal_impl_vk::cmd_draw_indirect_mesh_task() {
//    //vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
//    // vkCmdDrawMeshTasksEXT
//    return gal_error_code::SUC;
//}

// gal_error_code gal_impl_vk::cmd_draw_mesh_task(gal_command_list command) {
//     //vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
//     if (command != nullptr) {
//         return gal_error_code::ERR;
//     }
...
//     return gal_error_code::SUC;
// }
gal_error_code gal_impl_vk::cmd_copy_texture(gal_command_list command) {
    //vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
    if (command != nullptr) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::cmd_copy_buffer(gal_command_list command, gal_buffer src, gal_buffer dst, uint64 src_offset,
...
gal_error_code gal_impl_vk::cmd_fill_buffer(gal_command_list command) {
    if (command != nullptr) {
        return gal_error_code::ERR;
    }
    //vkCmdFillBuffer();
...
gal_error_code gal_impl_vk::cmd_fill_texture(gal_command_list command) {
    if (command != nullptr) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::cmd_upload_buffer(gal_command_list command) {
    if (command != nullptr) {
        return gal_error_code::ERR;
    }
    return gal_error_code::SUC;
}
gal_error_code gal_impl_vk::cmd_upload_texture(gal_command_list command) {
    if (command != nullptr) {
        return gal_error_code::ERR;
    }
    return gal_error_code::SUC;
}
gal_error_code gal_impl_vk::cmd_update_subresources(gal_command_list command, gal_texture dst, gal_buffer src,
    ...
    if (gal_tf_is_single_plane(fmt) == 0u) {
        return gal_error_code::ERR;
    }

    stl::vector<VkBufferImageCopy> regions(subresource_count);

    for (uint32 i = 0; i < subresource_count; i++) {
        if ((descs + i) == nullptr) {
            Log::log(gal_vk_logger, loglevel::error, "invalid subsource desc");
            return gal_error_code::ERR;
        }
        gal_texture_subresource_desc &desc = descs[i];
        uint32 width = _max<uint32>(1, dst->m_desc.width >> desc.mip_level);
        uint32 height = _max<uint32>(1, dst->m_desc.height >> desc.mip_level);
        uint32 depth = _max<uint32>(1, dst->m_desc.depth >> desc.mip_level);
...
gal_error_code gal_impl_vk::cmd_copy_texture_to_buffer(gal_command_list command) {
    if (command != nullptr) {
        return gal_error_code::ERR;
    }
...
gal_error_code gal_impl_vk::cmd_set_shading_rate() { return gal_error_code::SUC; }

gal_error_code gal_impl_vk::add_queue(gal_context context, gal_queue_desc *desc, gal_queue *queue) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    ...
    vk_queue *vk_q = bd::memory::alloc<gal::vk_queue>();
    if (vk_q == nullptr) {
        return gal_error_code::ERR;
    }
    *vk_q = *t_queue;
...
gal_error_code gal_impl_vk::remove_queue(gal_context context, gal_queue queue) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    if (queue != gal_null) {
        vk_queue *vk_q = reinterpret_cast<vk_queue *>(queue);
        vk_ctx->queues[utils_to_vk_queue_index(queue->m_type)].used = false;
        bd::memory::afree(vk_q);
        queue = gal_null;
    }
    return gal_error_code::SUC;
}

gal_error_code gal_impl_vk::queue_submit(gal_queue queue, gal_queue_submit_desc *desc) {
    //vk_context *vk_ctx = reinterpret_cast<vk_context *> (context);
    vk_queue *vk_q = reinterpret_cast<vk_queue *>(queue);

    uint32 cmdCount = desc->cmd_count;
    gal_command_list *ppCmds = desc->cmds;
    gal_fence fence = desc->pSignalFence;
    uint32 wait_semaphore_count = desc->wait_semaphore_count;
    uint32 signal_semaphore_count = desc->signal_semaphore_count;

    // some parameter check
    // ...
    //

    stl::vector<VkCommandBuffer> cmds;
    for (uint32 i = 0; i < cmdCount; ++i) {
        cmds.push_back(reinterpret_cast<vk_command_list *>(ppCmds[i])->m_command);
    }

    stl::vector<VkSemaphore> wait_semaphores;
    stl::vector<VkPipelineStageFlags> wait_masks;
    for (uint32 i = 0; i < wait_semaphore_count; ++i) {
    ...
    stl::vector<VkSemaphore> signal_semaphores;
    for (uint32 i = 0; i < signal_semaphore_count; ++i) {
...
gal_error_code gal_impl_vk::queue_present(gal_queue queue, gal_queue_present_desc *desc) {
...
gal_error_code gal_impl_vk::acquire_next_image(gal_context context, gal_swap_chain swap_chain,
...
gal_error_code gal_impl_vk::wait_queue(gal_queue queue) {
    vk_queue *vk_q = reinterpret_cast<vk_queue *>(queue);
...
//gal_error_code gal_impl_vk::create_srvuav(gal_context context, gal_texture texture)
//gal_error_code gal_impl_vk::create_srvuav(gal_context context, gal_buffer buffer)
//{
...
//}
// pipeline
//gal_error_code gal_impl_vk::create_shader(gal_context context, gal_shader_desc *desc, gal_shader *_shader) {
//    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
//    *_shader = reinterpret_cast<gal_shader>(bd::memory::alloc<vk_shader>());
//    vk_shader *vk_s = reinterpret_cast<vk_shader *>(*_shader);
//    VkShaderModuleCreateInfo create_info{};
//    create_info.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
//    create_info.codeSize = desc->size;
//    create_info.pCode = reinterpret_cast<uint32 *>(desc->data);
...
//    return gal_error_code::SUC;
//}
//gal_error_code gal_impl_vk::destroy_shader(gal_context context, gal_shader _shader) {
//    if (_shader != gal_null) {
//        vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
//        vk_shader *vk_s = reinterpret_cast<vk_shader *>(_shader);
