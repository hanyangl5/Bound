...
gal_error_code vk_create_instance(gal_desc *gal_desc, gal_context *context) {
    ...
    bd::vector<const char *> required_instance_layers;
    bd::vector<const char *> required_instance_extensions;
    ...
    bd::vector<VkLayerProperties> available_layers(available_layer_count);
    bd::vector<VkExtensionProperties> available_extensions(available_extension_count);
...
gal_error_code vk_create_device(gal_desc *gal_desc, gal_context *context) {
    ...
    bd::vector<const char *> required_device_extensions;
    bd::vector<const char *> required_device_layers;
    ...
    bd::vector<VkPhysicalDevice> physical_devices(physical_device_count);
    ...
    // pick gpu
    auto pick_gpu = [&](bd::array<uint32, 3> &queue_family_indices) -> gal_error_code {
        for (const auto &physical_device : physical_devices) {
            ...
            bd::vector<VkQueueFamilyProperties> queue_family_properties(queue_family_count);
            ...
            bd::vector<VkLayerProperties> available_device_layers(available_layer_count);
            ...
            bd::vector<VkExtensionProperties> available_device_extensions(available_extension_count);
    ...
    bd::array<uint32, 3> queue_family_indices;
    ...
    bd::set<uint32> queue_family_indices_set(queue_family_indices.begin(), queue_family_indices.end());
    bd::vector<VkDeviceQueueCreateInfo> device_queue_create_info(queue_family_indices_set.size());
    uint32 idx = 0;
    bd::array<uint32, 3> required_queue_family_indices;
...
#error PLATFORM NOT SUPPORTED
    ...
    bd::vector<VkSurfaceFormatKHR> formats;
    ...
    bd::vector<VkPresentModeKHR> modes;
    ...
    bd::array<VkPresentModeKHR, preferredModeCount> preferredModeList = {
    ...
    bd::array<VkCompositeAlphaFlagBitsKHR, 4> compositeAlphaFlags = {
...
gal_error_code vk_get_pipeline_cache_data(gal_context context, gal_pipeline_cache _pipeline_cache, uint64 *_size,
                                          void *_data) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);
    vk_pipeline_cache *vk_pc = reinterpret_cast<vk_pipeline_cache *>(_pipeline_cache);
...
gal_error_code vk_create_graphics_pipeline(gal_context context, gal_pipeline_desc *desc, gal_pipeline *pipeline) {
    ...
    uint32 stage_count = 0;
    bd::array<VkPipelineShaderStageCreateInfo, 5> stages;
    ...
    bd::array<VkVertexInputBindingDescription, MAX_VERTEX_BINDINGS> input_bindings{};
    uint32 input_attribute_count = 0;
    bd::array<VkVertexInputAttributeDescription, MAX_VERTEX_ATTRIBS> input_attributes{};
    ...
    bd::array<VkPipelineColorBlendAttachmentState, MAX_RENDER_TARGET_ATTACHMENTS> cbAtt{};
    ...
    bd::array<VkDynamicState, 5> dyn_states = {
...
gal_error_code vk_get_descriptor_set(gal_context context, gal_descriptor_set_desc *desc, uint32 set_count,
    ...
    bd::vector<VkDescriptorSet> vk_sets;
    for (uint32 i = 0; i < set_count; i++) {
        vk_sets.push_back(vk_ds[i].set);
    }
    bd::vector<VkDescriptorSetLayout> set_layouts;
...
gal_error_code vk_update_descriptor_set(gal_context context, gal_descriptor_set_update_desc *update_desc,
    ...
    bd::vector<VkDescriptorUpdateData> descriptor_updates;
...
gal_error_code vk_create_root_signature(gal_context context, gal_root_signature_desc *desc,
    ...
    bd::array<VkDescriptorSetLayoutCreateInfo, MAX_DESCRIPTOR_SET_COUNT> set_layouts_ci;
    //bd::array<uint32, MAX_DESCRIPTOR_SET_COUNT> set_binding_offsets;
    bd::vector<VkDescriptorSetLayoutBinding> bindings;
    //bd::vector<uint32> binding_prefix_sum(refl->sets.size(), );
    bd::vector<uint32> binding_offsets(refl->sets.size());
    uint32 total_binding_count = 0;
    bd::map<uint32, uint32> set_index_map;
    ...
    bd::vector<VkPushConstantRange> push_constants;
    ...
    bd::vector<bd::hash_map<VkDescriptorType, uint32>> pool_sizes;
    ...
    if (!refl->sets.empty()) {
        uint32 max_set_index =
            (*std::max_element(refl->sets.begin(), refl->sets.end(),
                               [](uint32 a, uint32 b) { return ((a >> 16) & 0x0000ffff) < ((b >> 16) & 0x0000ffff); }) >>
             16) &
            0x0000ffff;
    ...
    //bd::vector<VkDescriptorUpdateTemplateEntry> descriptor_update_template_entries();
    //descriptor_update_template_entries.resize(total_binding_count);
    bd::vector<bd::vector<VkDescriptorUpdateTemplateEntry>> descriptor_update_template_entries;
...
gal_error_code vk_wait_fences(gal_context context, gal_fence *fences, uint32 count) {
    vk_context *vk_ctx = reinterpret_cast<vk_context *>(context);

    
    bd::vector<VkFence> submit_fences;
    ...
    if (!submit_fences.empty()) {
        result = vkWaitForFences(vk_ctx->device, static_cast<uint32>(submit_fences.size()), submit_fences.data(), VK_TRUE,
                                 UINT64_MAX);
        if (result != VK_SUCCESS) {
            return gal_error_code::ERR;
        }
...
gal_error_code vk_cmd_set_scissor(gal_command_list command, int32 x, int32 y, uint32 width, uint32 height) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code vk_cmd_set_stencil_reference_value(gal_command_list command, uint32 val) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code vk_cmd_resource_barrier(gal_command_list command, uint32 buffer_barrier_count,
    ...
    bd::vector<VkBufferMemoryBarrier> bbs;
    ...
    bd::vector<VkImageMemoryBarrier> tbs;
    ...
    if (!bbs.empty() || !tbs.empty()) {
        vkCmdPipelineBarrier(vk_cmd->m_command, srcStageMask, dstStageMask, 0, 0, nullptr, static_cast<uint32>(bbs.size()),
                             bbs.data(), static_cast<uint32>(tbs.size()), tbs.data());
    }
    return gal_error_code::SUC;
}
...
gal_error_code vk_cmd_bind_vertex_buffer(gal_command_list command, uint32 vertex_buffer_count, gal_buffer *vertex_buffers,
                                         [[maybe_unused]] uint32 *strides, uint64 *offsets) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
    ...
    bd::array<VkBuffer, MAX_VERTEX_BUFFER_BINDING> buffers;
    bd::array<VkDeviceSize, MAX_VERTEX_BUFFER_BINDING> vb_offsets;
...
gal_error_code vk_cmd_bind_push_constant(gal_command_list command, gal_root_signature root_signature, const char *name,
    ...
    auto it = vk_rs->resource_map.find(bd::str(name));
    if (it == vk_rs->resource_map.end() || ((it->second >> 16) & 0x01) == 0) {
        return gal_error_code::ERR;
    }
    vkCmdPushConstants(vk_cmd->m_command, vk_rs->pipeline_layout, VK_SHADER_STAGE_ALL, 0, static_cast<uint32>(size), data);
    return gal_error_code::SUC;
}
...
gal_error_code vk_cmd_dispatch(gal_command_list command, uint32 group_count_x, uint32 group_count_y, uint32 group_count_z) {
    vk_command_list *vk_cmd = reinterpret_cast<vk_command_list *>(command);
...
gal_error_code vk_cmd_draw_instanced(gal_command_list command, uint32 vertex_count, uint32 first_vertex, uint32 instance_count,
...
gal_error_code vk_cmd_draw_indexed_instanced(gal_command_list command, uint32 index_count, uint32 first_index,
...
gal_error_code vk_cmd_copy_buffer(gal_command_list command, gal_buffer src, gal_buffer dst, uint64 src_offset,
...
gal_error_code vk_cmd_update_subresources(gal_command_list command, gal_texture dst, gal_buffer src,
    ...
    bd::vector<VkBufferImageCopy> regions(subresource_count);
...
gal_error_code vk_queue_submit(gal_queue queue, gal_queue_submit_desc *desc) {
    ...
    bd::vector<VkCommandBuffer> cmds;
    for (uint32 i = 0; i < cmdCount; ++i) {
        cmds.push_back(reinterpret_cast<vk_command_list *>(ppCmds[i])->m_command);
    }

    bd::vector<VkSemaphore> wait_semaphores;
    bd::vector<VkPipelineStageFlags> wait_masks;
    ...
    bd::vector<VkSemaphore> signal_semaphores;
