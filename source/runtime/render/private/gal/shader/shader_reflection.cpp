...
inline void read_resource_size(const spirv_cross::Compiler &compiler, const spirv_cross::Resource &resource,
    ...
    size_t array_size = 1;
    shader_resource.size = static_cast<uint32>(compiler.get_declared_struct_size_runtime_array(spirv_type, array_size));
}
...
void compiled_shader::create_shader_reflection_from_spirv() {
    ...
    //count += static_cast<uint32>(resources.stage_inputs.size());  // inputs
    //count += static_cast<uint32>(resources.stage_outputs.size()); // outputs
    //
    //// shader resources
    //count += static_cast<uint32>(resources.uniform_buffers.size());   // constant buffers
    //count += static_cast<uint32>(resourstces.orage_buffers.size());   // uav buffers
    //count += static_cast<uint32>(resources.separate_images.size());   // textures
    //count += static_cast<uint32>(resources.separate_samplers.size()); // samplers
    //count += static_cast<uint32>(resources.storage_images.size());    // uav textures
    ...
    // stage output
    for (uint32 i = 0; i < static_cast<uint32>(resources.stage_outputs.size()); ++i) {
...
void compiled_shader_group::create_pipeline_reflection() {
    ...
    m_pipeline_reflection = bd::memory::alloc<pipeline_reflection>();
    ...
    if (m_b_same_root_signature) {
        ...
        //    bd::vector<std::pair<const char *, ShaderResource>>(refl->resources.begin(), refl->resources.end());
    ...
    //uint32 vertexStageIndex = ~0u;
    //uint32 hullStageIndex = ~0u;
    //uint32 domainStageIndex = ~0u;
    //uint32 geometryStageIndex = ~0u;
    //uint32 pixelStageIndex = ~0u;
    ...
    //        for (uint32 j = 0; j < pSrcRef->mShaderResourceCount; ++j) {
    //            bool unique = true;
    //
    ...
    //            for (uint32 k = 0; k < resourceCount; ++k) {
    ...
    //        for (uint32 j = 0; j < pSrcRef->mVariableCount; ++j) {
    //            bool unique = true;
    ...
    //            for (uint32 k = 0; k < variableCount; ++k) {
    ...
    //            // look for parent
    //            for (uint32 j = 0; j < resourceCount; ++j) {
...
//        return;
//
//    for (uint32 i = 0; i < pReflection->mStageReflectionCount; ++i)
