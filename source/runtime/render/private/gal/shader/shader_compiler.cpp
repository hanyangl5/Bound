...
//struct FileNode {
//    bool need_compile;
//    bd::vector<std::filesystem::path> header_files;
//};
//
...
//                        bd::hash_map<std::filesystem::path, FileNode> &dependency_map) {
//    FileNode current_node;
//    auto text = io::read_text_file(bd::str{path.string()});
...
//    auto s_last_mod_time = bd::str{std::to_string(to_time_t(last_mod_time))};
...
//    } else {
//        io::write_text_file(bd::str{cached_path.string()}, s_last_mod_time.data(),
...
//                 bd::hash_set<std::filesystem::path> &visited_vertices,
//                bd::hash_map<std::filesystem::path, FileNode> &node_map) {
//
...
//    Container::HashMap<std::filesystem::path, std::pair<bd::str, bool>> shader_texts(
...
//            bd::str output_file_name = bd::str{path.filename().string()} + "." +
//                                                 bd::str{pos->str().substr(0, 2)} + ".hsb"; // add api
...
//    //tbb::parallel_for(tbb::blocked_range<uint32>(0, static_cast<uint32>(shader_compilation_settings.size())),
//    //                  [&shader_compilation_settings, &shader_texts, &settings](const tbb::blocked_range<uint32> &r) {
//    //                      for (uint32 v = r.begin(); v < r.end(); v++) {
